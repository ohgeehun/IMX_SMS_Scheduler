...
public class PropertyRead {
	...
	public void readConfigFile() {
		try {
			...
			NodeList testlist = nList.item(0).getChildNodes();
			for (int temp = 0; temp < nList.getLength(); temp++) {
				Node nNode = nList.item(temp);
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {

					Element eElement = (Element) nNode;
					// System.out.println("conn_ip : " +
					// nList.item(0).getChildNodes().item(1).getTextContent());
			...
			NodeList bind_nList = doc.getElementsByTagName("SMS_SET_BIND_VALUE");
			{
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					Element eElement = (Element) nNode; // 왜있는지 모르겠음
					...
					if (childlist.getLength() > 0) {
						 ...
						 * childlist.getLength(); i++) { System.out.println((i + 1) + " : " +
						 * childlist.item(i).getNodeType()); System.out.println((i + 1) + " : " +
						 * childlist.item(i).getNodeValue()); System.out.println((i + 1) + " : " +
						 * childlist.item(i).getNodeName()); System.out.println((i + 1) + " : " +
						...
						for (int j = 0; j < childlist.getLength(); j++) {
							...
							/*
							 * System.out.println(Node.TEXT_NODE + "," + childlist.item(j).getNodeType());
							 */
							if (Node.TEXT_NODE != childlist.item(j).getNodeType()) {
								...
								bindVariablesMap.put(childlist.item(j).getNodeName(),
										childlist.item(j).getTextContent());
							}
						}
					}
	...
	private static String getTagValue(String sTag, Element eElement) {
		...
		/*
		 * Node nValue1 = eElement.getElementsByTagName(sTag).item(0).getFirstChild();
		 */
		...
		/*
		 * System.out.println("nValue1.getNodeValue() : " + nValue1.getNodeValue());
	...
	//
	// for (int temp = 0; temp < nList.getLength(); temp++) { // 같은 엘리먼트가 여러개일 경우,
	// 여러번 반복해서 수집하기 위한 반복
	//
	// Node nNode = nList.item(temp);
	// if (nNode.getNodeType() == Node.ELEMENT_NODE) {
